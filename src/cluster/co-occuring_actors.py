...
"jerry brown","abu dhabi","greg abbott","las vegas","pope francis","superstorm sandy","john bel edwards","santa barbara","vladimir putin"
,"seth borenstein","john kerry","george w bush","malcolm turnbull","khalid al-falih","indian ocean","tony abbott","asia pacific",
...
def load_actor_occ():
  ...
  actors_df = spark.read.parquet(config.OUTPUT_PATH+"/gkg_filtered_5themes.parquet").select("V1Persons", "V1Organizations")
  ...
  actors_df.repartition(1).write.mode('overwrite').csv(path, header=True, sep=',')
