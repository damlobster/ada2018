...
mentions_df = spark.read.parquet(config.OUTPUT_PATH+"/mentions.parquet").select("MentionIdentifier","MentionTimeDate","GLOBALEVENTID")
# Read the parquet of gkg and select the document identifier and tone
gkg_df = spark.read.parquet(config.OUTPUT_PATH+"/gkg_filtered_5themes.parquet").select("V2DocumentIdentifier","V1Tone")
...
events_df = spark.read.parquet(config.OUTPUT_PATH+"/events.parquet").select("GLOBALEVENTID","Actor1Geo_CountryCode","Actor2Geo_CountryCode")
...
mentions_events_df = mentions_df.join(countries_df,mentions_df.GLOBALEVENTID==countries_df.GLOBALEVENTID)

print("join mention/event done")

# Join the mention and the gkg table
mentions = mentions_events_df.join(gkg_df,mentions_events_df.MentionIdentifier==gkg_df.V2DocumentIdentifier)
...
res.write.mode('overwrite').parquet(config.OUTPUT_PATH+"tone_mentions_5themes_europe.parquet")